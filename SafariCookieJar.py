    ...
    with open(binarycookies, "rb") as cookies_file:
        ...
        for page in [cookies_file.read(ps) for ps in page_sizes]:
            ...
            for offset in cookie_offsets:
                ...
                content_offsets = dict(zip(offset_values,
                ...
                cookie['expiry_time'] = unpack('<d', cookie_bytes.read(8))[0]+mac_epoch
                cookie['last_access_time'] = unpack('<d', cookie_bytes.read(8))[0]+mac_epoch
                ...
                if url in CookieJar:
                    if path in CookieJar[url]:
                        CookieJar[url][path][name] = cookie
                    else:
                        CookieJar[url][path] = { name: cookie }
                else:
                    CookieJar[url] = { path: { name: cookie} }
